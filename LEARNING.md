...
> Nginx is a web server and reverse proxy that can also be used as a load balancer. It handles client requests and can serve static content directly or forward requests to backend applications.

### Challenge 2

What's the difference between `Web Server` and `Reverse Proxy`?

> A web server can serve HTTP content directly, such as static files. A reverse proxy receives client requests and forwards them to backend servers or applications, then returns their responses to the client.
...
```
what work does every directtive?

> This is an Nginx server block. This Nginx file uses for add a new configuration (server block) to Nginx.

Work of every directive:
```Bash
listen 80 -> port 80.
server_name minishop.local -> host.
root /var/www/minishop -> root directory.
location / -> It defines how requests matching the `/` URI location are handled.
try_files $uri $uri/ =404; -> Nginx checks file or directory according to URI and if it doesn't find, Nginx returns `404` error code.
```
...
> `restart` stops and starts the Nginx service again, while `reload` tells Nginx to reload its configuration witout fully restarting the service.
...
> `sites-available` contains available site configurations, while `sites-enabled` contains the configurations that are enabled, usually through symbolic links to files in `sites-available`.
...
sites-enabled/
└── minishop -> ../sites-available/minishop
```

### Challenge 6

If `curl http://minishop.local` has `502 Bad Gateway` error, what do you check?

> If Nginx returns 502, I would first check the Nginx error log, then verify that the upstream backend is running and reachable. I would also check the `proxy_pass` configuration and Docker or network connectivity if the backend is containerized.
